 ...
 def set_value(self,valuenum,value,publish=True): # implement if GPIO used!!
  if float(valuenum)<=self.valuecount and float(valuenum)>0:
   ...
   if self.formulaoption: # handle formulas
    tval = False
    if len(self.formula[valuenum-1])>0 and isformula(self.formula[valuenum-1]):
     tval = parseformula(self.formula[valuenum-1],float(value))
     if tval!=None:
      rval = tval
   ...
   if self.valuenames[valuenum-1]!= "":
    try:
     rulesProcessing(self.taskname+"#"+self.valuenames[valuenum-1]+"="+str(rval),pglobals.RULE_USER)
    except: # maximum recursion depth?
     pass
   if self.senddataoption and publish:
    self.plugin_senddata(pchangedvalue=valuenum)
   try:
    if self._writecallback is not None:
     self._writecallback(self.taskindex,valuenum)
   except:
    pass
