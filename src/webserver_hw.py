...
try:
 ...
 pass
from web import parse_qs, awrite

async def handle_hw(request,response,chunk):
  ws.navMenuIndex=3
  ws.TXBuffer = ""
...
#       settings.HW[tname] = int(ws.arg(tname,responsearr))
   ...
   if "p0" in responsearr:
    for p in range(gpiohelp.MAX_GPIO):
     pv = -1
     try:
...
#   settings.loadpinout()
  ...
  ws.addHtml("<p><div><a class='menu2' href='hardware'>Main</a> | <a class='menu2' href='hardware?method=i2c'>I2C</a> | <a class='menu2' href='hardware?method=spi'>SPI</a> | <a class='menu2' href='hardware?method=uart'>UART</a>") 
  if settings.ISLAN:
   ws.addHtml("| <a class='menu2' href='hardware?method=lan'>LAN</a>")
  ws.TXBuffer += "</div><form name='frmselect' method='post'><table class='normal'>"
  if await awrite(response,ws.TXBuffer,chunked=chunk):
   ws.TXBuffer = ""
  if method=="":
#   ws.addFormSeparator(3)
   ws.addFormHeader("Main")
   options = ['0','80','125','130','160','240']
   optionvalues = [0,80000000, 125000000, 130000000, 160000000, 240000000]
   try:
    afreq = int(libhw.getfreq())
   except:
    afreq = 0
...
#     print("LAN failed ",e)#debug
    ...
    ws.addFormHeader("LAN WizNET interface")
    ws.addFormCheckBox("Enable LAN","lanif", settings.Settings["LANIF"])
    ws.addFormNote("SPI MOSI=GP19, MISO=GP16, CS=GP17, SCK=GP18, RST=G20, INT=GP21")
