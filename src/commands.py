...
def addtoTime(basetime, deltastr): # -1h +2h -10m +3m ...
 ...
 try:
  if sign==1:
   return basetime[0], basetime[1], basetime[2], basetime[3] + hours, basetime[4] + mins
  else:
   return basetime[0], basetime[1], basetime[2], basetime[3] - hours, basetime[4] - mins
 except:
  return basetime
   ...
   if svname==SysVars[0]: #%systime%	01:23:54
    ...
    if suntimesupported==1:
     try:
      ...
      tz = 0
     try:
      sun = Sun(settings.AdvSettings["Latitude"],settings.AdvSettings["Longitude"],tz)
