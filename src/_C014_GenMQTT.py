...
import settings
import commands
from commands import doExecuteCommand, parseruleline, rulesProcessing
mqttok = False
try:
...
class Controller(controller.ControllerProto):
 ...
 def controller_init(self,enablecontroller=None):
  ...
  self._inch, state = parseruleline(self.inchannel)    # replace global variables
  self._outch, state = parseruleline(self.outchannel)
...
#    commands.rulesProcessing("DomoMQTT#Connected",pglobals.RULE_SYSTEM)
 ...
 def disconnect(self):
   ...
   self.laststatus = 0
   try:
    rulesProcessing("GenMQTT#Disconnected",pglobals.RULE_SYSTEM)
   except:
    pass
  ...
  if self.enabled and self.initialized:
   ...
   if res != self.laststatus:
    if res==0:
     ...
     rulesProcessing("GenMQTT#Connected",pglobals.RULE_SYSTEM)
    self.laststatus = res
